...
.defines(function() {
  EntityEnemy = ig.Entity.extend({
    ...
    health: 1,
    stompSound: new ig.Sound('media/audio/smb_stomp.ogg', true),
    kickSound: new ig.Sound('media/audio/smb_kick.ogg', true),
    bumpSound: new ig.Sound('media/audio/smb_bump.ogg', true),
    init: function(x, y, settings) {
      this.parent(x, y, settings);
    },
    update: function() {
      if (ig.game.player && (ig.game.player.pos.x+ig.system.width/1.2>this.pos.x && ig.game.player.pos.x-ig.system.width/1.2<this.pos.x)) {
        this.onscreen=true;
      } else {
        this.onscreen = false;
      }
      if (this.onscreen) {
        ...
        if (this.safetyCollide && ig.game.collisionMap.getTile(
