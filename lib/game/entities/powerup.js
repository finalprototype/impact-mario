...
EntityPowerup = ig.Entity.extend({
  ...
  appearsSound: new ig.Sound('media/audio/smb_powerup_appears.ogg',false),

  init: function(x, y, settings) {
    this.parent(x, y, settings);
    this.eColour = ig.game.levelType>0?1:0;
    this.addAnim( 'mushroom', 10, [8+(this.eColour*1)] );
    this.addAnim( 'life', 10, [10] );
    this.addAnim( 'poison', 10, [11] );
    this.addAnim( 'flower', 0.05, [0+(this.eColour*4),1+(this.eColour*4),2+(this.eColour*4),3+(this.eColour*4),2+(this.eColour*4),1+(this.eColour*4)] );
